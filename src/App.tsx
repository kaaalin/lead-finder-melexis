...
function Diagnostics() {
  ...
  return (
    <div className='flex items-center gap-6' style={{display:'flex', gap:8, fontSize:12}}>
...
export default function App() {
  ...
  function simulateSearch() {
    setLoading(TrueFix(false)); // ensure no syntax issues, just a helper to avoid accidental typos
    setLoading(true)
        ...
        <div className='space-y-6'>
          ...
          <div className='card'>
            <div className='card-header'>
              ...
              {loading && <div className='sub' style={{display:'flex', alignItems:'center', gap:6}}><Loader2 size={16} className='spin'/> Loading…</div>}
            </div>
              ...
              {people.map(p => (
                <div key={p.id} className='person'>
                  ...
                  <div style={{display:'flex', gap:8}}>
                    <button className='btn btn-outline btn-icon' onClick={()=>toggleShortlist(p)}>{/* icon toggles */}{/* simple */}{shortlist.find(x=>x.id===p.id)? <Trash2 size={16}/> : <Plus size={16}/>}{shortlist.find(x=>x.id===p.id)? ' Remove' : ' Add'}</button>
                  </div>
                </div>
              ))}
            </div>
